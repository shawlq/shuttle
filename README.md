# shuttle：通过 Git 仓在 Windows 与 Linux 之间传文本

两台机器处于不同局域网、彼此不可直连，但都能访问同一远程 Git 时，可用本工具把一段文本经仓库同步到另一端。

## 原理

在仓库中约定一个载荷文件（默认 `shuttle/shuttle_payload.txt`）。**发送**会先 `pull --rebase`，写入该文件，`git add` / `commit` / `push`；**接收**会先 `pull`，再读取该文件内容。依赖本机已安装的 `git` 命令，无额外 Python 依赖。
...
| `SHUTTLE_PAYLOAD_REL` | 可选，载荷相对仓库根的路径，默认 `shuttle/shuttle_payload.txt`。 |
