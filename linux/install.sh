...
SHUTTLE_SRC_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
...
安装（在仓库根目录执行）:
  ...
  sudo env PREFIX=/usr/local bash linux/install.sh
  bash linux/install.sh --prefix ~/.local
...
卸载:
  ...
  bash linux/install.sh --uninstall --prefix ~/.local
...
_remove_path_snippet() {
  local rc="$1"
  [[ -f "${rc}" ]] || return 0
  if grep -q '# shuttle PATH begin' "${rc}" 2>/dev/null; then
    sed -i '/# shuttle PATH begin/,/# shuttle PATH end/d' "${rc}"
  fi
...
_append_path_snippet() {
  ...
  {
    echo ""
    echo "# shuttle PATH begin"
    echo "export PATH=\"${bindir}:\$PATH\""
    echo "# shuttle PATH end"
  } >>"${rc}"
}
...
LIBDIR="${PREFIX}/lib/shuttle"
BINDIR="${PREFIX}/bin"
CONFIG_DIR="${LIBDIR}/linux/.shuttle.env"
...
for _d in linux common win; do
  "${maybe_sudo[@]}" rm -rf "${LIBDIR}/${_d}"
  "${maybe_sudo[@]}" cp -a "${SHUTTLE_SRC_ROOT}/${_d}" "${LIBDIR}/"
done
...
tmp_cfg="$(mktemp)"
{
  echo "# Generated by linux/install.sh — 勿提交到 Git 仓库"
...
CONFIG="${CONFIG_PATH}"
if [[ ! -f "\$CONFIG" ]]; then
  echo "缺少配置文件: ${CONFIG_PATH}，请重新运行 linux/install.sh。" >&2
  exit 1
fi
set -a
# shellcheck disable=SC1090
...
exec python3 -m linux.shuttle_cmd "\$@"
EOF
...
echo "  程序库:   ${LIBDIR}（linux、common、win）"
echo "  本地克隆: ${CLONE_DIR}"
echo "  配置文件: ${CONFIG_PATH}"
echo "  卸载记录: ${manifest}"
echo ""
